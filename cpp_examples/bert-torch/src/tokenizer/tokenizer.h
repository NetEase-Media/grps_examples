...
public:
  Tokenizer(const std::string& vocab_path, bool do_lower_case = true, bool codepoint_level = true);
  ...
  std::vector<SizeT> convert_tokens_to_ids(const std::vector<std::string>& tokens, bool add_cls_sep = false) const;
  ...
  void wordpiece_tokenize(const std::string& text, std::vector<std::string>& tokens, std::vector<SizeT>& offsets) const;
...
protected:
  ...
  void build_pos_map(const char* str, SizeT len, std::vector<SizeT>& pos_map) const;
  void build_index_map(const std::string& text, std::vector<SizeT>& byte2index) const;
