...
public:
  ...
  struct node {
    ...
    node(const int base__ = 0, const int check_ = 0) : base_(base__), check(check_) {}
  ...
  // interfance
  template <typename T>
  void exactMatchSearch(const char* key, T& result) const {
    result = exactMatchSearch<T>(key);
  }
  template <typename T>
  ...
  value_type& update(const char* key, size_t& from, size_t& pos, size_t len, value_type val, T& cf) {
    if (!len && !from) _err(__FILE__, __LINE__, "failed to insert zero-length key\n");
  ...
  int build(size_t num, const char** key, const size_t* len = 0, const value_type* val = 0) {
    for (size_t i = 0; i < num; ++i) update(key[i], len ? len[i] : std::strlen(key[i]), val ? val[i] : value_type(i));
    return 0;
  }
  template <typename T>
  ...
  int next(size_t& from, size_t& len, const size_t root = 0) {
    ...
    return c ? begin(from = static_cast<size_t>(_array[from].base()) ^ c, ++len) : CEDAR_NO_PATH;
  }
...
private:
  ...
  static void _realloc_array(T*& p, const int size_n, const int size_p = 0) {
    ...
    if (!tmp) std::free(p), _err(__FILE__, __LINE__, "memory reallocation failed\n");
    p = static_cast<T*>(tmp);
    static const T T0 = T();
  ...
  void _initialize() { // initilize the first special block
    ...
    _array[0] = node(0, -1);
#endif
    for (int i = 1; i < 256; ++i) _array[i] = node(i == 1 ? -255 : -(i - 1), i == 255 ? -1 : -(i + 1));
...
#endif
  void _set_result(result_type* x, value_type r, size_t = 0, size_t = 0) const { *x = r; }
  ...
  int _resolve(size_t& from_n, const int base_n, const uchar label_n, T& cf) {
    ...
    uchar* const last =
      flag ? _set_child(first, base_n, _ninfo[from_n].child, label_n) : _set_child(first, base_p, _ninfo[from_p].child);
      ...
      if (NUM_TRACKING_NODES) // keep the traversed node updated
        for (size_t j = 0; tracking_node[j] != 0; ++j)
          if (tracking_node[j] == static_cast<size_t>(to_)) {
            tracking_node[j] = static_cast<size_t>(to);
            break;
          }
    }
