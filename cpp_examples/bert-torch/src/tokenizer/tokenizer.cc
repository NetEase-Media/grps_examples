...
std::vector<Token> BasicTokenizer::basic_tokenize(const std::string& text) const {
  std::vector<Token> tokens;
  basic_tokenize(text, tokens);
  return tokens;
}

void BasicTokenizer::tokenize(const std::string& text, SizeT pos, std::vector<Token>& tokens) const {
  int32_t unicode = 0;
  ...
  while (i < len) {
    if (isascii(data[i])) {
      ...
      if (n == 0) {
        ...
        if (!isCntrl(word[0])) {
          last_state = false;
          if (!isspace(word[0])) tokens.emplace_back(pos + start, pos + i, std::string(word, n));
        }
        n = 0;
      }
      ...
      word[n++] = data[i++];
      while (i < len && (data[i] & 0xC0) == 0x80) word[n++] = data[i++];
      word[n] = '\0';
      start = i - n;
      ...
      if ((unicode >= 0x4E00 && unicode <= 0x9FFF) || (unicode >= 0x3400 && unicode <= 0x4DBF) ||
          (unicode >= 0x20000 && unicode <= 0x2A6DF) || (unicode >= 0x2A700 && unicode <= 0x2B73F) ||
          (unicode >= 0x2B740 && unicode <= 0x2B81F) || (unicode >= 0x2B820 && unicode <= 0x2CEAF) ||
          (unicode >= 0xF900 && unicode <= 0xFAFF) || (unicode >= 0x2F800 && unicode <= 0x2FA1F) ||
        ...
        if (_do_lower_case) {
          ...
          if (token.size() == 1 && !isalnum(token[0]) && !isCntrl(token[0])) {
            last_state = false;
            if (!isspace(token[0])) tokens.emplace_back(pos + start, pos + i, token);
            n = 0;
            continue;
          }
...
std::string BasicTokenizer::normalize(const uint8_t* str) const {
  ...
  while (i < len) {
    if (isascii(norm[i])) {
      ...
      if (strcmp(utf8proc_category_string(unicode), "Mn") != 0) result.append((char*)(norm + i - n), n);
      n = 0;
    }
  }
  free(norm);
...
void Tokenizer::load_vocab(const std::string& vocab_path) {
  ...
  if (!ifs.is_open()) throw std::invalid_argument(vocab_path + " can not be opened for loading!");
  std::string token;
  while (std::getline(ifs, token))
    if (!token.empty()) _vocab->insert(token);
  ifs.close();
}
...
void Tokenizer::build_pos_map(const char* str, SizeT len, std::vector<SizeT>& pos_map) const {
  int32_t unicode = 0;
  uint8_t* ch = new uint8_t[8];
  ...
  while (cur < len) {
    if (isascii(str[cur])) {
      ...
      if (_do_lower_case) {
        m = utf8proc_encode_char(utf8proc_tolower(unicode), ch);
        ch[m] = '\0';
        for (SizeT i = 0; i < NFD_codepoint_number(ch); i++) pos_map.emplace_back(val);
      } else
        pos_map.emplace_back(val);
...
SizeT Tokenizer::NFD_codepoint_number(const uint8_t* str) const {
  ...
  while (i < len) {
    if (isascii(norm[i])) {
      ...
      utf8proc_iterate(norm + i - n, n, &unicode);
      if (strcmp(utf8proc_category_string(unicode), "Mn") != 0) c++;
      n = 0;
    }
  }
  free(norm);
...
WidthT Tokenizer::get_num_bytes_of_utf8_char(const char* str, SizeT len) const {
  SizeT cur = 1;
  WidthT num_bytes = 1;
  while (cur < len && (str[cur++] & 0xC0) == 0x80) num_bytes++;
  return num_bytes;
}
...
void Tokenizer::build_index_map(const std::string& text, std::vector<SizeT>& byte2index) const {
  auto data = text.c_str();
...
SizeT Tokenizer::search(const char* str, SizeT len, SizeT index) const {
  SizeT cur_bytes = 0, cur_index = 0;
  while (cur_bytes < len) {
    if (cur_index == index) return cur_bytes;
    cur_bytes += get_num_bytes_of_utf8_char(str + cur_bytes, len - cur_bytes);
    cur_index++;
  }
  if (cur_index == index) return len;
  return _npos;
}
...
std::vector<std::string> Tokenizer::convert_ids_to_tokens(const std::vector<SizeT>& input_ids) const {
  std::vector<std::string> tokens;
  tokens.reserve(input_ids.size());
  for (SizeT i = 0; i < input_ids.size(); i++) tokens.emplace_back(get_token(input_ids[i]));
  return tokens;
}

std::vector<SizeT> Tokenizer::convert_tokens_to_ids(const std::vector<std::string>& tokens, bool add_cls_sep) const {
...
void Tokenizer::convert_tokens_to_ids(const std::vector<std::string>& tokens,
  ...
  if (add_cls_sep) input_ids.emplace_back(_cls_id);
  for (SizeT i = 0; i < tokens.size(); i++) input_ids.emplace_back(get_id(tokens[i]));
  if (add_cls_sep) input_ids.emplace_back(_sep_id);
}
...
void Tokenizer::wordpiece_tokenize(const std::string& text,
  ...
  if (_codepoint_level) build_index_map(text, byte2index);
  ...
  for (SizeT i = 0; i < base_tokens.size(); i++) {
    ...
    while (cur < token.size()) {
      ...
      num = cur > 0 ? get_codepoint_number(prefix) - 2 : get_codepoint_number(prefix);
...
std::vector<std::string> Tokenizer::wordpiece_tokenize(const std::string& text) const {
  std::vector<std::string> tokens;
...
void Tokenizer::encode(const std::string& text,
  ...
  if (truncation && input_ids.size() > max_length) {
    ...
    offsets.resize(n);
    if (add_cls_sep) input_ids[max_length - 1] = _sep_id;
    if (input_ids.capacity() > max_length * 32) std::vector<SizeT>(input_ids).swap(input_ids);
  }
...
void Tokenizer::encode(const std::vector<std::string>& texts,
  ...
  if (num_threads <= 1)
    for (SizeT i = 0; i < n; i++)
      encode(texts[i], input_ids[i], token_type_ids[i], attention_mask[i], offsets[i], add_cls_sep, truncation,
             max_length);
  else {
...
#pragma omp parallel for num_threads(num_threads)
  ...
  // padding
  SizeT max_seq_len =
    std::accumulate(input_ids.begin(), input_ids.end(), 0,
                    [](size_t len, const std::vector<SizeT>& input) { return std::max(len, input.size()); });
