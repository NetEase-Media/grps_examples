## resnet-50-torch-batching

torch版本resnet-50图片分类服务。通过自定义converter（基于opencv开发），可以实现接口层支持图片的输入，直接返回label输出。支持打开dynamic
...
docker build -t resnet50_torch_online:1.0.0 -f docker/Dockerfile .

# 启动docker容器
docker run -itd --runtime=nvidia --name="resnet50_torch_online" -p 8030:8030 -p 8031:8031 resnet50_torch_online:1.0.0
...
'''输出结果如下：
{'status': {'code': 200, 'msg': 'OK', 'status': 'SUCCESS'}, 'str_data': 'Samoyed'}
'''
...
'''输出结果如下：
I20231220 17:46:24.054205 16370 grpc_client.cc:52] Predict label: Samoyed, latency: 71906 us
'''
...
'''输出结果如下：
I1220 17:49:12.433334 18215 brpc_client.cc:59] Predict label: Samoyed, latency: 71355 us
'''
