...
tensorrt版本resnet-50图片分类服务。通过自定义converter（基于opencv开发），可以实现接口层支持图片的输入，直接返回label输出。支持打开dynamic
...
# 启动docker容器
docker run -itd --runtime=nvidia --name="resnet50_trt_online" -p 8020:8020 -p 8021:8021 resnet50_trt_online:1.0.0
